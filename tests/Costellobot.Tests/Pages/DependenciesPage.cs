...
public sealed class DependenciesPage(IPage page) : AppPage(page)
{
    ...
    public async Task DenyDependencyAsync(string ecosystem, string id, string version)
    {
        await Page.SelectOptionAsync(Selectors.DenyEcosystem, ecosystem);
    ...
    public sealed class DeniedDependencyItem : Item
    {
        ...
        public async Task<string> EcosystemAsync()
        {
            var element = await SelectAsync(Selectors.DeniedDependencyEcosystem);
            return await element.GetAttributeAsync("title") ?? string.Empty;
        }

        public async Task<string> IdAsync()
            => await StringAsync(Selectors.DeniedDependencyId);

        public async Task<string> VersionAsync()
            => await StringAsync(Selectors.DeniedDependencyVersion);
    ...
    private sealed class Selectors
    {
        ...
        internal const string DeniedDependencyEcosystem = "[class*='denied-dependency-ecosystem']";
        internal const string DeniedDependencyId = "[class*='denied-dependency-id']";
        internal const string DeniedDependencyItem = "[class*='denied-dependency-row']";
        internal const string DeniedDependencyVersion = "[class*='denied-dependency-version']";
        ...
        internal const string DenySubmit = "id=deny-dependency-submit";
    }
}
