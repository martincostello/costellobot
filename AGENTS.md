# Coding Agent Instructions for Costellobot

## Repository Overview

**Costellobot** is a .NET ASP.NET Core web application that provides GitHub automation services. The application handles GitHub webhooks to automatically approve dependency updates, manage deployments, and perform repository maintenance tasks across multiple GitHub repositories. The version of .NET used can be determined from the `global.json` file in the root of the repository, which specifies the SDK version to use for building and running the application. Costellobot runs as an Azure Container Instance backed by Azure Service Bus, Blob Storage, Table Storage, and Key Vault.
...
- **Languages**: C# (the latest version associated with the .NET SDK version in use), TypeScript, PowerShell
...
**ALWAYS use PowerShell Core (pwsh)** to run PowerShell (`.ps1`) scripts - it works on macOS, Linux and Windows.
...
Frontend assets (TypeScript/CSS) must be built separately from `src/Costellobot/`:

```bash
npm install
npm run build      # compile + format + lint + test
npm run compile    # TypeScript/webpack only
...
- `src/Costellobot/Handlers/` — One handler per GitHub webhook event type; all implement `IHandler`
- `src/Costellobot/Registries/` — Package registry integrations (NuGet, npm, Docker, PyPI, RubyGems, GitHub Actions, etc.) implementing `IPackageRegistry`
- `src/Costellobot/DeploymentRules/` — Pluggable deployment approval rules implementing `IDeploymentRule`
- `src/Costellobot/Octokit/` — GitHub API client wrappers and credential stores
- `src/Costellobot/Authorization/` — ASP.NET Core auth handlers for admin and health probe access
- `src/Costellobot/scripts/ts/` — TypeScript source; compiled to `wwwroot/static/` via webpack
- `src/Costellobot/styles/` — CSS source files
- `src/Costellobot.AppHost/` — .NET Aspire orchestration host
- `tests/Costellobot.Tests/` — Integration tests (xUnit v3 + Playwright UI tests)
- `perf/Costellobot.Benchmarks/` — BenchmarkDotNet benchmarks
...
All tests in `tests/Costellobot.Tests/` are integration tests using `WebApplicationFactory<Program>`. Key infrastructure:

- `AppFixture` — Hosts the full app in-process; replaces Azure Service Bus with `InMemoryServiceBusClient`, HTTP calls intercepted via `JustEat.HttpClientInterception`
- `IntegrationTests<T>` — Base class providing helpers: `PostWebhookAsync`, `RegisterGetAccessToken`, `RegisterReview`, `AssertTaskNotRun`, etc.
- `Builders/` — Fluent builders for GitHub API response objects (`PullRequestBuilder`, `RepositoryBuilder`, etc.)
- `Drivers/` — Scenario-level test setup helpers (e.g., `PullRequestDriver.ForDependabot()`)
- `Shouldly` — Assertion library; `NSubstitute` for mocking where needed

Tests verify async side-effects (e.g., whether a PR was approved) using `TaskCompletionSource` with a `ResultTimeout` of 1 second.

### Request Flow

1. GitHub sends a webhook POST to `/github-webhook`
2. The webhook is validated (HMAC-SHA256 signature), serialized, and enqueued to Azure Service Bus via `GitHubWebhookService`
3. `GitHubWebhookDispatcher` dequeues messages and dispatches to the appropriate handler via `HandlerFactory`
4. Handlers use `IGitHubClientForInstallation` (Octokit) to interact with the GitHub API
...
- `GitHub.Apps` — Per-app configuration (AppId, PrivateKey, ClientId)
- `GitHub.Installations` — Maps installation IDs to apps
- `Webhook.TrustedEntities` — Allow-lists: `Users` (trusted bot accounts), `Dependencies` (trusted package name regexes), `Publishers` (trusted publisher accounts per registry)
- `Webhook.Approve` / `Webhook.Automerge` / `Webhook.Deploy` — Feature flags (all `false` by default)
- `Webhook.IgnoreRepositories` — Repos that should never be auto-processed
...
- Logging uses compile-time source generation (`[LoggerMessage]` attributes) inside a nested `private static partial class Log`
- Configuration uses `IOptionsMonitor<T>` with strongly-typed options classes
- Central NuGet package management via `Directory.Packages.props`; never specify versions in individual `.csproj` files
- Markdown links use `[text][link]` reference syntax, not `[text](url)` inline syntax
- When modifying GitHub Actions workflows, always pin action versions to a specific commit SHA or tag

## Frontend Notes

- TypeScript sources: `src/Costellobot/scripts/ts/`
- Entry point: `src/Costellobot/scripts/main.ts`
- UI uses Bootstrap (CSS/layout) and FontAwesome (icons)
- If `.cshtml` changes affect element IDs, classes, or structure that TypeScript manipulates, update the TypeScript too
- Output goes to `src/Costellobot/wwwroot/static/` (gitignored build artifacts)
